...
class SheetsProcessor:
        ...
        try:
            ...
            for row in values:
                ...
                if "/spreadsheets/d/" in url:
                    ...
                    hyperlinks.append({"url": url, "spreadsheet_id": spreadsheet_id})
    ...
    def get_spreadsheet_title(self, spreadsheet_id):
        """스프레드시트 제목 가져오기"""
        try:
            info = self.service.spreadsheets().get(spreadsheetId=spreadsheet_id, fields="properties.title").execute()
    ...
    def get_sheet_names(self, spreadsheet_id):
        ...
        try:
            result = api_call_with_backoff(
                self.service.spreadsheets().get(spreadsheetId=spreadsheet_id, fields="sheets.properties.title").execute
            )
    ...
    def get_info_data(self, spreadsheet_id):
        ...
        try:
            ...
            batch_request = (
                ...
                .values()
                .batchGet(
                    spreadsheetId=spreadsheet_id, ranges=[rng for rng, _ in ranges], valueRenderOption="FORMATTED_VALUE"
                )
            )
    ...
    def read_sheet(self, spreadsheet_id, range_name=None):
        ...
        try:
            ...
            result = api_call_with_backoff(
                self.service.spreadsheets()
                .values()
                .get(spreadsheetId=spreadsheet_id, range="'WORKSHEET'!A1:AZ200", valueRenderOption="FORMATTED_VALUE")
                .execute
            )
    ...
    def create_summary_sheet(self, spreadsheet_id, summary_data, sheet_name="Summary"):
        ...
        try:
            ...
            # 데이터 준비
            headers = ["카테고리", "완료 작업 수", "총 작업 수", "진행률 (%)", "NaN 발생률 (%)", "Overtime 발생률 (%)"]
            rows = []
    ...
    def read_info_sheet(self, spreadsheet_id):
        ...
        try:
            ...
            result = api_call_with_backoff(
                self.service.spreadsheets()
                .values()
                .get(spreadsheetId=spreadsheet_id, range="'정보판'!A1:Z100", valueRenderOption="FORMATTED_VALUE")
                .execute
            )
