...
from .settings import BASE_WORK_HOURS, DEFAULT_TASKS, HOLIDAYS, MODEL_TIME_FACTORS, WORK_SCHEDULE, logger
    ...
    def calculate_working_hours(self, start_time, end_time):
        ...
        while current_time < end_time:
            ...
            if day_of_week >= 5 or self.is_holiday(current_date):
                current_time = datetime.combine(current_date + timedelta(days=1), time(*WORK_SCHEDULE["WORK_START"]))
                continue

            # 작업 시작/종료 시간 설정
            work_start = datetime.combine(current_date, time(*WORK_SCHEDULE["WORK_START"]))
            work_end = datetime.combine(current_date, time(*WORK_SCHEDULE["WORK_END"]))
...
class TaskAnalyzer:
    ...
    def analyze_data(self, df):
        ...
        df["워킹데이 소요 시간"] = df.apply(
            lambda row: self.calculator.calculate_working_hours(row["시작 시간"], row["완료 시간"]), axis=1
        )
...
def analyze_occurrence_rates(df, task_total_time, model_name, tolerance=2, mech_partner=None, elec_partner=None):
    """NaN & Overtime 발생률 분석"""
    ...
    for category in ["기구", "전장", "검사", "마무리", "TMS"]:
        ...
        ot_count = len(category_tasks[category_tasks["워킹데이 소요 시간"] > tolerance_time])
